...
async def respond_to_journey_press_prompt_start(
    args: CreateJourneyEventRequest[EventRequestDataT],
    authorization: Optional[str] = Header(None),
):
    """Indicates the user began pressing down on a journey press prompt, but
    only if the journey has a press prompt and the user is not already pressing
    ...
    async with Itgs() as itgs:
        ...
        if is_already_pressing:
            return Response(
                content=StandardErrorResponse[CREATE_JOURNEY_EVENT_409_TYPES](
                    type="impossible_event_data",
                    message=(
                        "A press prompt start response can only be sent if the user "
                        "is not already pressing down on the press prompt."
                    ),
                ).json(),
                headers={
                    "Content-Type": "application/json; charset=utf-8",
