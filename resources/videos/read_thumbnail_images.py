...
class VideoThumbnailUserSource(BaseModel):
...
class VideoThumbnailFrameSource(BaseModel):
...
class VideoThumbnail(BaseModel):
    ...
    source: VideoThumbnailSource = Field(description="The source of the thumbnail")
...
VideoThumbnailSortOption = Union[
...
class VideoThumbnailFilter(BaseModel):
    uid: Optional[FilterTextItemModel] = Field(
        None, description="the uid of the video thumbnail row"
    )
...
class ReadVideoThumbnailRequest(BaseModel):
    filters: VideoThumbnailFilter = Field(
        default_factory=lambda: VideoThumbnailFilter.model_validate({}),
        description="the filters to apply",
    )
    sort: Optional[List[VideoThumbnailSortOption]] = Field(
...
class ReadVideoThumbnailResponse(BaseModel):
    items: List[VideoThumbnail] = Field(
        description="the items matching the filters in the given sort"
    )
    next_page_sort: Optional[List[VideoThumbnailSortOption]] = Field(
...
async def read_video_thumbnails(
    ...
    *,
    ...
    """Lists out video thumbnails from a table matching the pattern formed by
    `course_video_thumbnails`.

    This requires standard authorization for a user with admin access
    """
    sort = [srt.to_result() for srt in (args.sort or [])]
    sort = cleanup_sort(VIDEO_THUMBNAIL_SORT_OPTIONS, sort, ["uid"])
    async with Itgs() as itgs:
        ...
        items = await raw_read_video_thumbnails(
        ...
        if items and any(s.after is not None for s in sort):
            ...
            rev_items = await raw_read_video_thumbnails(
        ...
        return Response(
            content=ReadVideoThumbnailResponse.__pydantic_serializer__.to_json(
...
async def raw_read_video_thumbnails(
    itgs: Itgs,
    ...
    *,
    ...
    video_thumbnail_images = Table(table_name)
    image_files = Table("image_files")

    query: QueryBuilder = (
        Query.from_(video_thumbnail_images)
        .select(
            video_thumbnail_images.uid,
            video_thumbnail_images.source,
            image_files.uid,
            image_files.created_at,
            video_thumbnail_images.last_uploaded_at,
        )
        .join(image_files)
        .on(image_files.id == video_thumbnail_images.image_file_id)
    )
    qargs = []

    def pseudocolumn(key: str) -> Term:
        if key == "uid":
            return video_thumbnail_images.field("uid")
        elif key == "image_file_created_at":
            return image_files.field("created_at")
        elif key == "source_type":
            return Function(
                "json_extract", video_thumbnail_images.field("source"), "$.type"
            )
        elif key == "source_frame_number":
            return Function(
                "json_extract",
                video_thumbnail_images.field("source"),
                "$.frame_number",
            )
        elif key == "last_uploaded_at":
            return video_thumbnail_images.field("last_uploaded_at")
...
def item_pseudocolumns(item: VideoThumbnail) -> dict:
