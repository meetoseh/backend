...
@router.post(
    ...
    responses={
        ...
        "409": {
            "description": "The provided journal entry is not in the correct state for this operation. For example, the user hasn't responded to the reflection question, so we cannot summarize the entry",
...
async def ensure_summary(
    ...
    add a summary, one is generated and added to the end of the entry. This will
    use a journal client key as an additional encryption layer.
    ...
    async with Itgs() as itgs:
        ...
        if queue_job_result.type == "bad_state":
            if queue_job_result.subtype == "already-has-summary":
                queue_job_at = time.time()
                queue_job_result = (
                ...
                await handle_warning(
                    ...
                    "a summary, but it is not in the right state to have one",
                )
