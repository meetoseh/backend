    ...
    with temp_file(".bak") as backup_file:
        ...
        with open(backup_file, "rb") as f:
            await files.upload(
                ...
                key=f"s3_files/backup/database/timely/0247_fix_transitions-pre-{int(time.time())}.bak",
                sync=True,
            )
...
WHERE
    (? IS NULL OR uid > ?)
    AND slug IN ('hold_to_continue', 'choose_a_feeling', 'journal_chat', 'audio_interstitial')
ORDER BY uid ASC
LIMIT ?
            """,
    ...
    with temp_file(".bak") as backup_file:
        ...
        with open(backup_file, "rb") as f:
            await files.upload(
                f,
                bucket=files.default_bucket,
                key=f"s3_files/backup/database/timely/0247_fix_transitions-post-{int(time.time())}.bak",
                sync=True,
            )
