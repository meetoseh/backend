...
class CourseLogoFilter(BaseModel):
...
class ReadCourseLogoRequest(BaseModel):
    filters: CourseLogoFilter = Field(
        default_factory=lambda: CourseLogoFilter.model_validate({}),
        description="the filters to apply",
    )
    sort: Optional[List[CourseLogoSortOption]] = Field(
...
class ReadCourseLogoResponse(BaseModel):
    items: List[CourseLogo] = Field(
        description="the items matching the filters in the given sort"
    )
    next_page_sort: Optional[List[CourseLogoSortOption]] = Field(
...
@router.post(
    "/search",
    response_model=ReadCourseLogoResponse,
    responses=STANDARD_ERRORS_BY_CODE,
)
async def read_course_logos(
    args: ReadCourseLogoRequest,
    authorization: Annotated[Optional[str], Header()] = None,
):
    """Lists out course logos

    This requires standard authorization for a user with admin access
    """
    sort = [srt.to_result() for srt in (args.sort or [])]
    sort = cleanup_sort(COURSE_LOGO_SORT_OPTIONS, sort, ["uid"])
    async with Itgs() as itgs:
        ...
        items = await raw_read_course_logos(
        ...
        if items and any(s.after is not None for s in sort):
            ...
            rev_items = await raw_read_course_logos(itgs, filters_to_apply, rev_sort, 1)
            if rev_items:
        ...
        return Response(
            content=ReadCourseLogoResponse.__pydantic_serializer__.to_json(
                ReadCourseLogoResponse(
                    items=items,
                    next_page_sort=(
...
async def raw_read_course_logos(
    itgs: Itgs,
    ...
    course_logo_images = Table("course_logo_images")
    image_files = Table("image_files")

    query: QueryBuilder = (
        Query.from_(course_logo_images)
        .select(
            course_logo_images.uid,
            image_files.uid,
            image_files.created_at,
            course_logo_images.last_uploaded_at,
        )
        .join(image_files)
        .on(image_files.id == course_logo_images.image_file_id)
    )
    qargs = []

    def pseudocolumn(key: str) -> Term:
        if key == "uid":
            return course_logo_images.field("uid")
            ...
            return course_logo_images.field("last_uploaded_at")
...
def item_pseudocolumns(item: CourseLogo) -> dict:
