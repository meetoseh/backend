...
async def read_user_streak(
    itgs: Itgs, *, sub: str, prefer: Literal["model"]
) -> UserStreak: ...


@overload
async def read_user_streak(
    itgs: Itgs, *, sub: str, prefer: Literal["bytes"] = "bytes"
) -> bytes: ...


async def read_user_streak(
    itgs: Itgs, *, sub: str, prefer: Literal["model", "bytes"] = "bytes"
    ...
    for _ in range(1):  # used so we can break out / goto
        async with basic_redis_lock(
            itgs, f"users:{sub}:streak:lock".encode("utf-8"), spin=True, timeout=3
        ):
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async with Itgs() as itgs:
            ...
            joined = await read_user_streak(itgs, sub=user_sub, prefer="model")
