...
async def sync_user_stripe_revenue_cat(itgs: Itgs, *, user_sub: str) -> None:
    ...
    if await users.lib.entitlements.is_revenue_cat_outage(itgs):
        logger.warning(
            f"Syncing {user_sub} stripe<->revenue cat is disabled due to revenue cat outage"
        )
        raise Exception("Revenue cat outage")
    ...
    for customer in customers:
        ...
        while True:
            ...
            subs = await asyncio.wait_for(
                run_in_threadpool(subs.next_page, api_key=stripe_sk), timeout=60
            )
    ...
    if need_purge_caches:
        ...
        await users.lib.entitlements.publish_purge_message(
            itgs, user_sub=user_sub, min_checked_at=time.time()
        )
