"""Adds blurred background images to all journeys, then triggers the job to redo processing"""
...
async def up(itgs: Itgs) -> None:
    ...
    # add blurred background images to journeys
    await cursor.executemany2(
        (
            ...
            INSERT INTO journeys_new (
                ...
                instructor_id, title, description, journey_subcategory_id, prompt, created_at, deleted_at
            )
            SELECT
                id, uid, audio_content_file_id, background_image_file_id, background_image_file_id,
                instructor_id, title, description, journey_subcategory_id, prompt, created_at, deleted_at
            FROM journeys
            """,
            "DROP TABLE journeys",
    ...
    # add blurred version to journey_background_images
    await cursor.executemany2(
        (
            "PRAGMA foreign_keys = OFF",
            ...
            INSERT INTO journey_background_images_new (
                id, uid, image_file_id, blurred_image_file_id, uploaded_by_user_id, last_uploaded_at
            )
            SELECT
                id, uid, image_file_id, image_file_id, uploaded_by_user_id, last_uploaded_at
