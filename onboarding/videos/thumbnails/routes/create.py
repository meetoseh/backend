...
class CreateOnboardingVideoThumbnailRequest(BaseModel):
...
async def create_onboarding_video_thumbnail(
    args: CreateOnboardingVideoThumbnailRequest,
    authorization: Annotated[Optional[str], Header()] = None,
):
    """Starts the process to create a new onboarding video thumbnail. At least
    1920x1080 is required, though larger, especially for the height, is
    preferred as these videos are shown in portrait mode on high-resolution
    devices.

    Thumbnails created by extracting specific frames are produced automatically when
    a video is uploaded, so this is primarily for more specific thumbnails.
    ...
    async with Itgs() as itgs:
        ...
        res = await start_upload(
            ...
            success_job_name="runners.process_onboarding_video_thumbnail",
            success_job_kwargs={
