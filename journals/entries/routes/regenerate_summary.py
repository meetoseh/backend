...
from journals.entries.routes.regenerate_reflection_question import (
    ERROR_409_TYPES,
    ERROR_BAD_STATE_RESPONSE,
    RegenerateReflectionQuestionRequest,
)
from models import (
...
@router.post(
    ...
    responses={
        ...
        "409": {
            "description": "The provided journal entry is not in the correct state for this operation. For example, the indicated entry is not a summary",
...
async def regenerate_summary(
    ...
    """Regenerates the summary at the given entry counter within the
    ...
    async with Itgs() as itgs:
        ...
        if entry_auth_result.result.user_sub != std_auth_result.result.sub:
            await handle_warning(
                ...
                f"User {std_auth_result.result.sub} tried to regenerate a summary, but the JWT provided "
        ...
        queue_job_result = (
            await lib.journals.start_journal_chat_job.regenerate_journal_entry_summary(
                itgs,
                journal_entry_uid=args.journal_entry_uid,
                user_sub=std_auth_result.result.sub,
        ...
        if queue_job_result.type == "bad_state":
            await handle_warning(
                ...
                "but they did not indicate a summary",
            )
