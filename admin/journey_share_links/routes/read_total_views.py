...
class ReadTotalViewsResponse(BaseModel):
    value: int = Field(
        description="The total number of journey share link views since the beginning of time"
    )
...
async def read_total_views(authorization: Annotated[Optional[str], Header()] = None):
    """Reads the total number of journey share link views since the
    ...
    async with Itgs() as itgs:
        ...
        result_raw = await redis.get(b"stats:journey_share_links:views:count")
