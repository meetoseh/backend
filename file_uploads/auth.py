"""Provides utility functions for working with file upload jwts"""
...
class SuccessfulAuthResult:
    file_upload_uid: str
    """The UID of the file upload which they have access too"""
...
async def auth_presigned(itgs: Itgs, authorization: Optional[str]) -> AuthResult:
    ...
    the JWT should be signed with `OSEH_FILE_UPLOAD_JWT_SECRET`, have the audience
    `oseh-file-upload`, and have an iat and exp set and valid.
    ...
    secret = os.environ["OSEH_FILE_UPLOAD_JWT_SECRET"]
    ...
    try:
        claims = jwt.decode(
            ...
            audience="oseh-file-upload",
            issuer="oseh",
        )
    except Exception as e:
        if not isinstance(e, jwt.exceptions.ExpiredSignatureError):
            await handle_error(e, extra_info="failed to decode file upload jwt")
        return AuthResult(
            result=None,
            error_type="invalid",
    ...
    return AuthResult(
        result=SuccessfulAuthResult(file_upload_uid=claims["sub"], claims=claims),
        error_type=None,
        error_response=None,
    )
...
async def create_jwt(itgs: Itgs, file_upload_uid: str, duration: int = 1800) -> str:
    """Produces a JWT for the given file upload by uid. The returned JWT will
    be acceptable for `auth_presigned`.

    Args:
        itgs (Itgs): The integrations to use to connect to networked services
        file_upload_uid (str): The uid of the file upload to create a JWT for
    ...
    return jwt.encode(
        ...
        os.environ["OSEH_FILE_UPLOAD_JWT_SECRET"],
        algorithm="HS256",
    )
