...
async def get_cached_raw_web_playlist(
    ...
    raw_bytes = local_cache.get(f"content_files:exports:web:{uid}")
...
async def get_cached_raw_web_playlist_as_response(
    ...
    raw_result: Union[bytes, io.BytesIO] = local_cache.get(
        f"content_files:exports:web:{uid}", read=True
    )
...
async def set_cached_raw_web_playlist(
    ...
    local_cache = await itgs.local_cache()
    local_cache.set(
        f"content_files:exports:web:{uid}", bytes(response.json(), "utf-8"), expire=900
    )
