    ...
    return [
        MergeQuery(
            query=(
                ...
                " users.id,"
                " users.sub "
                "FROM params, user_identities, users "
                "WHERE"
                " user_identities.provider = params.merging_provider"
                " AND user_identities.sub = params.merging_provider_sub"
                " AND users.id = user_identities.user_id"
                " AND users.sub <> params.original_user_sub"
                ")"
                # ---
                ", original_emails("
                " email"
                ...
                "SELECT"
                " user_email_addresses.email "
                "FROM user_email_addresses, users, params "
                "WHERE"
                " user_email_addresses.user_id = users.id"
                " AND users.sub = params.original_user_sub"
                ...
                "SELECT"
                " user_phone_numbers.phone_number "
                "FROM user_phone_numbers, users, params "
                "WHERE"
                " user_phone_numbers.user_id = users.id"
                " AND users.sub = params.original_user_sub"
                ...
                "SELECT"
                " EXISTS ("
                "  SELECT 1 FROM user_daily_reminders, users"
                "  WHERE"
                ...
                " ),"
                " EXISTS ("
                "  SELECT 1 FROM user_daily_reminders, merging_user"
                "  WHERE"
                "   user_daily_reminders.user_id = merging_user.id"
                "   AND user_daily_reminders.channel = 'email'"
                " ),"
                " EXISTS ("
                "  SELECT 1 FROM user_daily_reminders, users"
                ...
                " ),"
                " params.merge_at "
                "FROM params, merging_user, computed_context_1, computed_context_2, step_result, users "
                "WHERE"
